"""
COMPLETE FINAL ANALYTICAL DATASET — DEFINITIVE VERSION
All errors fixed including duplicate column save error.
Run: python3 climate_merge_final.py  (~20-30 min)
"""
...
print("=" * 65)
print("BUILDING COMPLETE ANALYTICAL DATASET")
print("=" * 65)
...
print(
    f"    N4:{len(b4):,}  LBW={b4['lbw'].mean():.4f}  has_bw={b4['has_bw'].mean():.1%}"
)
...
b5["caesarean"] = b5["m17"]
b5["size_perceived"] = b5["m18"]
b5["preg_months"] = np.nan
b5["preg_valid"] = False
b5["s220a"] = np.nan
b5["preterm"] = np.nan
b5["conception_cmc"] = np.nan
b5["conception_month"] = np.nan
b5["conception_year"] = np.nan
for col in [
    "t1_start_cmc",
    "t1_end_cmc",
    "t2_start_cmc",
...
print(
    f"    N5:{len(b5):,}  LBW={b5['lbw'].mean():.4f}  has_bw={b5['has_bw'].mean():.1%}"
)
print(f"    NOTE: N5 T3 climate will be 0% — s220a not available in NFHS-5")
print(
    f"    NOTE: N5 LBW is {b5['has_bw'].mean():.1%} — recorded for most recent birth only"
)
...
print(
    f"    Combined:{len(births):,}  N4={(births.wave==4).sum():,}  N5={(births.wave==5).sum():,}"
)
print(
    f"    LBW N4:{births[births.wave==4]['lbw'].mean():.4f}  N5:{births[births.wave==5]['lbw'].mean():.4f}"
)
...
print(
    f"    BMI     N4:{births[births.wave==4]['bmi'].notna().mean():.1%}  "
    f"N5:{births[births.wave==5]['bmi'].notna().mean():.1%}"
)

# ══════════════════════════════════════════════════════════════════════════════
# 6. NHS — select only KEY infrastructure columns, avoid duplicates
# ══════════════════════════════════════════════════════════════════════════════
print("\n[6] NHS infrastructure (key columns only)...")
...
# Select ONLY specific useful columns — avoids 265-column explosion and duplicates
NHS_COLS_KEEP = [
    "state_code_num",
    "nhs_wave_num",
...
print(f"    Coverage: {births[nhs_added[0]].notna().mean():.1%}" if nhs_added else "")
...
print(
    f"    preg_valid: {to_num(births['preg_valid']).eq(1).mean():.1%} (N4 only — s220a not in NFHS-5)"
)
...
print(
    f"    Birth-month done:{np.sum(~np.isnan(bm_anom)):,} ({np.mean(~np.isnan(bm_anom)):.1%})"
)

valid_t3 = (
    valid_bm & to_num(births["preg_valid"]).eq(1) & births["t3_start_cmc"].notna()
)
print(f"    T3 valid:{valid_t3.sum():,} (N4 only — no preg duration in N5)")
...
rep = max(1, total // 10)
print("    Processing T1/T2/T3 (~15 min)...")
for i in range(len(births)):
    if not valid_t3.iloc[i]:
        continue
    la, lo, rla, rlo = tlat[i], tlon[i], rlat[i], rlon_v[i]
    ...
    if n > 0:
        ...
        t3rf[i] = rf
        t3dr[i] = dr
    if not pd.isna(t1s[i]) and not pd.isna(t1e[i]):
        h = nn = 0
        ...
        if nn > 0:
            t1h35[i] = h
    if not pd.isna(t2s[i]) and not pd.isna(t2e[i]):
        h = nn = 0
...
print(f"    T3 done:{np.sum(~np.isnan(t3h35)):,}")
...
# ══════════════════════════════════════════════════════════════════════════════
# 10. FINAL VARIABLES + STANDARDISE
# ══════════════════════════════════════════════════════════════════════════════
print("\n[10] Final variables...")
...
print(
    f"  LBW N4:{births[births.wave==4]['lbw'].mean():.4f}  "
    f"N5:{births[births.wave==5]['lbw'].mean():.4f}"
)
print(f"  Districts:{births['district'].nunique()}  States:{births['state'].nunique()}")
print(f"  Years:{int(births['birth_year'].min())}–{int(births['birth_year'].max())}")
print(f"\n  Coverage (N4 / N5):")
for col, lbl in [
    ("lbw", "LBW"),
    ...
    ("t3_hot_days_35", "T3 hot days (N4 only)"),
    ("anaemic", "Anaemia"),
    ("bmi", "BMI"),
...
nc = [c for c in births.columns if c.startswith("nhs_")]
print(f"\n  NHS cols:{len(nc)}: {nc}")
print(f"\n  LBW by wealth:")
for q in [1, 2, 3, 4, 5]:
...
print(
  ...
  DATA LIMITATION NOTES:
    T3 climate (N5=0%):  NFHS-5 births recode has no s220a
                         (pregnancy duration). T3 requires
                         trimester windows. Use birth_month_tmax
                         for full-sample analysis with N5.
    LBW (N5=28%):        NFHS-5 records m19 only for the most
                         recent birth per woman. Other births
                         have missing weight — this is by design
                         in the DHS survey protocol.
    IMPLICATION:         LBW analysis uses N4 (has_bw=33%).
                         Mortality analysis uses both waves (100%).
                         Birth-month Tmax uses both waves (60%).
"""
)
...
if os.path.exists(old):
    shutil.copy(old, f"{PROCESSED}/final_analytical_dataset_N4only_backup.parquet")
    print("    Backup saved")
births.to_parquet(old, index=False, compression="snappy")
births.to_csv(f"{PROCESSED}/final_analytical_dataset.csv", index=False)
print(f"    Parquet:{os.path.getsize(old)/1e6:.0f} MB  Cols:{len(births.columns)}")
...
print(
  ...
  NFHS-4 births          ✅
  NFHS-5 births          ✅ (T3=N/A by design — no preg duration)
  Household both waves   ✅
  Individual/anaemia/BMI ✅
  NHS infrastructure     ✅ (key cols only — no duplicates)
  IMD Tmax T1/T2/T3      ✅ (N4) / birth-month (N4+N5)
  IMD Rainfall T3        ✅ (N4)
  ERA5 national trend    ✅

  Next: python3 final_correct.py
"""
)
